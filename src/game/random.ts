...
function sha256(input: string): Uint8Array {
  ...
  for (let offset = 0; offset < 8; offset += 1) {
    padded[padded.length - 1 - offset] = Number((bitLength >> BigInt(offset * 8)) & 0xffn);
  }
  ...
  for (let chunk = 0; chunk < padded.length; chunk += 64) {
    for (let i = 0; i < 16; i += 1) {
      ...
      w[i] = (padded[base] << 24) | (padded[base + 1] << 16) | (padded[base + 2] << 8) | padded[base + 3];
    }
...
export function deriveSeed64(input: string): bigint {
...
function splitmix64(seed: bigint): () => bigint {
  ...
  return () => {
    ...
    return mix64(state) & MASK_64;
  };
}

export function createRng(seed: string): () => number {
  const next = splitmix64(deriveSeed64(seed) || 1n);
  return () => Number(next() & MASK_32) / 4294967296;
}
